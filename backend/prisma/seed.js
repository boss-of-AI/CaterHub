...
async function main() {
    ...
    try {
        ...
        const createdDishes = await Promise.all(DISHES.map((d) => prisma.dish.create({ data: { name: d.name, category: d.category, isNonVeg: d.isNonVeg, cuisine: d.cuisine || null, isPremium: d.isPremium || false, isActive: true, tags: [] } })));
        const byCategory = (cat) => createdDishes.filter((d) => d.category === cat).map((d) => d.id);
